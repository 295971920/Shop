...
<body>
					...
					<div class="container_oc">
						<div class="row">
							<div id="content_oc" class="col-sm-12 view-product">
								<!-- 根据商品ID获取并显示商品信息 -->
								<%
									int typeSystem = 0; //保存商品类型的ID变量
									int ID = Integer.parseInt(request.getParameter("ID"));//保存商品名称变量
									if(ID >0){
										...
										float price = (float)0.0; //保存商品原价格的变量
										String picture = "";//保存商品图片变量
										String introduce = "";//保存商品描述的变量
										if(rs.next()){//如果找到对应商品的信息
											goodsName = rs.getString(2);//获取商品名称
											introduce = rs.getString(3);//获取商品的描述
											nowprice = rs.getFloat(4);//获取商品现价
											picture = rs.getString(5);//获取商品的图片
											price = rs.getFloat(6);//获取商品的原价
											typeSystem = rs.getInt(7);//获取商品的类别ID
										}
											conn.close();
								 %>
								<!-- 显示商品详细信息 -->
								<div class="row">
									<div class="col-xs-12 col-md-4 col-sm-4">
										<ul class="thumbnails" style="list-style: none">
											<li><a class="thumbnail" href="#"> <img src="../images/goods/56.jpg"></a></li>
										</ul>
									</div>
									<div class="col-xs-12 col-md-8 col-sm-8">
										<div style="margin-left: 30px; margin-top: 20px">
											<h1 class="product-title"><%=goodsName %></h1>
											<ul class="list-unstyled price"><li><h2><%=price%>元</h2></li></ul>
											<ul class="list-unstyled price"><li>原价: <%=nowprice %>元</li></ul>
								...
								<%
										...
										out.print("<script language='javascript'>alert('您的操作有误');"+"window.location.href='index.jsp';</script>");
									}
								 %>
								<!-- //显示商品详细信息 -->
								<!-- 显示相关商品 -->
	...
	<!-- //版权栏 -->
		...
		<script type="text/javascript">
			function addCart(){
				var num = $('#shuliang').val();//获取商品的输入数量
				//验证输入的数量是否合法
				if(num <1){
					alert('数量不能小于1！');
					return;
				}
				//调用添加购物车的页面；实现将该商品添加到购物车
				windows.location.href="car_add_jsp?goodsID=<%=ID%>&num="+num;
			}
		</script>
</body>
</html>
